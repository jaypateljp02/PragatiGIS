...
export default function ReportGenerator({ claims = [] }: ReportGeneratorProps) {
  ...
  const generatePDFReport = async () => {
    ...
    try {
      ...
      pdf.text('PragatiGIS Report', 20, currentY);
      currentY += 20;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        pdf.text('Generated by PragatiGIS Platform', 20, footerY);
      ...
      // Save the PDF
      const fileName = `pragati-report-${reportOptions.template}-${new Date().toISOString().split('T')[0]}.pdf`;
      pdf.save(fileName);
